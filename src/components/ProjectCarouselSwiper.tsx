...
function buildVideos(): string[] {
  const rest = [1,2,3,4,5,6,7,8,10,11,12,13]
  for (let i = rest.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [rest[i], rest[j]] = [rest[j], rest[i]]
  }
  const ts = Date.now()
  return [`/videos/Video-preview9.webm?t=${ts}`, ...rest.map(n => `/videos/Video-preview${n}.webm?t=${ts}`)]
}

export default function ProjectCarouselSwiper() {
